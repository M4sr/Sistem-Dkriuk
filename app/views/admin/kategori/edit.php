...
<div class="container-fluid px-4 py-3">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div>
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0" style="font-size: 12px;">
                        ...
                        <li class="breadcrumb-item active text-danger fw-bold" aria-current="page">Edit Kategori</li>
                    </ol>
                </nav>
            </div>
        </div>
        <div class="d-flex gap-2">
            ...
            <button type="submit" form="formEdit" class="btn btn-danger shadow-sm px-4 fw-bold rounded-3">Simpan Perubahan</button>
        </div>
    </div>
        ...
        <div class="col-lg-7">
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-body p-4">
                    ...
                    <form action="<?= BASEURL; ?>/admin/proses_edit_kategori" method="post" id="formEdit">
                        <input type="hidden" name="id" value="<?= $k['id']; ?>">
                        
                        <div class="mb-4">
                            <label class="form-label small fw-bold">Nama Kategori <span class="text-danger">*</span></label>
                            <input type="text" name="nama" id="inputNama" class="form-control rounded-3 py-2" value="<?= $k['nama']; ?>" placeholder="Contoh: Ayam Crispy" required>
                        ...
                        <div class="mb-4">
                            <label class="form-label small fw-bold d-flex justify-content-between">
                                Deskripsi (Opsional)
                                <span id="charCount" class="text-muted fw-normal" style="font-size: 10px;"><?= strlen($k['deskripsi'] ?? ''); ?>/255</span>
                            </label>
                            <textarea name="deskripsi" id="inputDeskripsi" class="form-control rounded-3" rows="4" placeholder="Keterangan singkat tentang kategori ini..." maxlength="255"><?= $k['deskripsi']; ?></textarea>
                        </div>

                        <div class="row mb-4">
                            <div class="col-md-6">
                                <label class="form-label small fw-bold">Urutan Tampil <span class="text-danger">*</span></label>
                                <input type="number" name="urutan" id="inputUrutan" class="form-control rounded-3 py-2" value="<?= $k['urutan']; ?>" required>
                        ...
                        <!-- Hidden Icon Input -->
                        <input type="hidden" name="ikon" id="inputIkon" value="<?= $k['ikon']; ?>">
                    </form>
                </div>
            </div>
        </div>
                ...
                <div class="card-body p-4">
                    ...
                    <div class="text-center py-4 px-3 mb-5 rounded-4 bg-light bg-opacity-50 border border-dashed">
                        <div class="mx-auto bg-danger bg-opacity-10 text-danger rounded-circle mb-3 d-flex align-items-center justify-content-center" style="width: 80px; height: 80px;">
                            <i id="previewIcon" class="fas fa-<?= !empty($k['ikon']) ? $k['ikon'] : 'drumstick-bite'; ?> fs-1"></i>
                        </div>
                        <h4 id="previewNama" class="fw-bold text-dark mb-2"><?= $k['nama']; ?></h4>
                        <p id="previewDeskripsi" class="text-muted small px-4 mb-3"><?= !empty($k['deskripsi']) ? $k['deskripsi'] : 'Keterangan singkat tentang kategori ini...'; ?></p>
                        <?php 
                        $status_class = ($k['status'] == 'aktif') ? 'bg-success text-success' : 'bg-secondary text-secondary';
                        ?>
                        <span id="previewStatus" class="badge <?= $status_class; ?> bg-opacity-10 rounded-pill px-3 py-1 fw-bold" style="font-size: 10px;"><?= ucfirst($k['status']); ?></span>
                    </div>
                    ...
                    <div class="icon-grid d-grid gap-2" style="grid-template-columns: repeat(5, 1fr);">
                        ...
                        foreach($icons as $icon) : 
                            $active_icon = ($k['ikon'] == $icon || (empty($k['ikon']) && $icon == 'drumstick-bite'));
                        ?>
                        <div class="icon-item border rounded-3 p-3 text-center cursor-pointer <?= $active_icon ? 'border-danger bg-danger bg-opacity-10 text-danger' : 'text-muted'; ?>" 
